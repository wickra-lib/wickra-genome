...
h <- wkgenome_new('{"features":[{"kind":"price","field":"close"}],"symbols":["AAA","BBB","CCC"],"normalize":"z_score","metric":"euclid","seed":24333}')
out <- wkgenome_command(h, '{"cmd":"version"}')
stopifnot(is.character(out), length(out) == 1L)
stopifnot(grepl("version", out, fixed = TRUE))

cat("wickra-genome R package smoke: ok (version ", v, ")\n", sep = "")

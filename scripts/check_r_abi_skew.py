...
takes the header and library from `WKGENOME_INC` / `WKGENOME_LIB`, supplied from
outside, and compiles the hand-written `src/wickra_genome.c` against whatever
...
This makes the pairing explicit:
  ...
  * Every `wickra_*` symbol `src/wickra_genome.c` calls must be declared in
    `bindings/c/include/wickra_genome.h`. A call to a symbol that is not
...
downloads a prebuilt `wickra-genome-c-<triple>.tar.gz` for the version in
...
HEADER = "bindings/c/include/wickra_genome.h"

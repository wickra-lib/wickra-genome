...
make, so this reads `wickra_genome_<name>` out of
`bindings/c/include/wickra_genome.h` and checks each language's public surface
...
HEADER = os.path.join(ROOT, "bindings", "c", "include", "wickra_genome.h")

# Exports that are ABI plumbing rather than a promise to callers.
ABI_ONLY = {"free"}

EXPORT = re.compile(r"\bwickra_genome_([a-z0-9_]+)\s*\(")
...
# file only re-exports `Genome` from the compiled module, so the methods
...
BINDINGS: dict[str, tuple[list[str], dict[str, str]]] = {
    "python": (
        ["bindings/python/src/lib.rs", "bindings/python/python/wickra_genome/__init__.py"],
        {
            "new": r"(?m)^\s*fn new\s*\(",
    ...
    "csharp": (
        ["bindings/csharp/WickraGenome/Genome.cs"],
        {
            "new": r"(?m)^\s*public Genome\s*\(",
    ...
    "go": (
        ...
        {
            "new": r"(?m)^func New\s*\(",
            "command": r"(?m)^func \([^)]*\*Genome\) Command\s*\(",
            "version": r"(?m)^func Version\s*\(",
        },
    ),
    "java": (
        ["bindings/java/src/main/java/org/wickra/genome/Genome.java"],
        {
            "new": r"(?m)^\s*public Genome\s*\(",
            "command": r"(?m)^\s*public [^\n]*\bcommand\s*\(",
            "version": r"(?m)^\s*public static [^\n]*\bversion\s*\(",
    ...
    "r": (
        ["bindings/r/R/genome.R", "bindings/r/NAMESPACE"],
        {
            "new": r"(?m)^wkgenome_new\s*<-\s*function",
            "command": r"(?m)^wkgenome_command\s*<-\s*function",
            "version": r"(?m)^wkgenome_version\s*<-\s*function",
        },
    ),
    ...
    "cpp": (
        ["bindings/c/include/wickra_genome.hpp"],
        {
            "new": r"\bGenome\s*\(\s*(?:const\s+)?std::string",
...
def main() -> int:
    ...
    if not exports:
        print(f"no wickra_genome_* exports found in {HEADER}", file=sys.stderr)
        return 1

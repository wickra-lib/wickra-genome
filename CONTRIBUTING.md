# Contributing to wickra-genome

Thanks for your interest. Issues, bug reports, ideas and pull requests are all
welcome at <https://github.com/wickra-lib/wickra-genome>. For larger changes,
open an issue first so we can agree on the approach.

## Orientation

- The core — the `GenomeSpec`, the per-symbol `SymbolState` fold, the
  `FeatureMatrix` and the `build` / `build_batch` entry points — lives in
  `crates/genome-core`. The spec is **data, not code**: a serde struct, so
  the same feature build crosses the C ABI and WASM unchanged.
- The reference consumer is `crates/genome-cli` (the `wickra-genome` binary).
